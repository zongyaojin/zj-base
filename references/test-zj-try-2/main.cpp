...
namespace zj {
namespace debug {
const std::string k_stdExceptionSite {"ZJ_STD_EXCEPTION_SITE"};
}
}
    ...
    ZjExceptionType t, const std::exception& e, const char* file, int line, const char* func, const std::string& fmt = "", Args&&... args)
{
    ...
    switch (t) {
        ...
        case ZjExceptionType::Bug: {
            if (msg == zj::debug::k_stdExceptionSite) {
                fmtMsg = fmt::format(
                    std::move(logFmt), file, line, func, fmt::format("std exception, type: [{}], what: [{}]", _ZJ_DEMANGLE(e), e.what()));
            }
            spdlog::error("{}", fmtMsg);
            throw ZjBug("caught from std exception, see trace");
        } break;
        default:
    ...
    do {                                                                                                                                   \
        ...
        switch (e) {                                                                                                                       \
            ...
            case ZjExceptionType::Fault:                                                                                                   \
                ...
                break;                                                                                                                     \
            default:                                                                                                                       \
                _ZJ_ASSERT(false, "incorrect exception type, code shouldn't reach here, check implementation logic");                      \
        }                                                                                                                                  \
    } while (0)
...
#define _ZJ_TRY(function)                                                                                                                  \
    do {                                                                                                                                   \
        try {                                                                                                                              \
            ...
            _ZjThrow(ZjExceptionType::Bug, e, __FILE__, __LINE__, __FUNCTION__, zj::debug::k_stdExceptionSite);                            \
        } catch (...) {                                                                                                                    \
